    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                ...
                InlineKeyboardButton(text="Помощь", callback_data="help:disclaimer"),
            ],
        ]
    )
...
def plot_actions(plot_id: int, tracking_enabled: bool) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Добавить извещение", callback_data=f"notice:add:{plot_id}"
                )
            ],
            [InlineKeyboardButton(text="Добавить аукцион", callback_data=f"auction:add:{plot_id}")],
            [
                InlineKeyboardButton(
                    text="Отключить контроль" if tracking_enabled else "Включить контроль",
                    callback_data=f"plot:toggle:{plot_id}",
                )
            ],
        ]
    )
